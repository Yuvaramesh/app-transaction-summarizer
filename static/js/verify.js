/* Ajeer – Recipient Risk Verification (ML-powered) */
...
senderSel.addEventListener("change", async () => {
  ...
  if (sender) {
    document.getElementById("badgeAge").textContent =
      sender.account_age_label || "—";
    document.getElementById("badgeTypical").textContent =
      `£${sender.typical_transfer_amount}`;
    document.getElementById("badgeLimit").textContent =
      `£${sender.monthly_limit_gbp}`;
  ...
  try {
    ...
    const res = await fetch(
      `/api/recipients?sender_id=${encodeURIComponent(sid)}`,
    );
    ...
    if (recipientsCache.length === 0) {
      recipientSel.innerHTML =
        '<option value="">No recipients for this sender</option>';
    } else {
...
recipientSel.addEventListener("change", () => {
  ...
  if (r) {
    ...
    document.getElementById("rbDays").textContent =
      `${r.days_since_added} days`;
...
function renderResult(data) {
  ...
  const scoreLabel =
    tier === "green" ? "Low" : tier === "amber" ? "Medium" : "High";
  ...
  // DB context strip — now includes ML model scores
  ...
  const chips = [
    ...
    [
      "Monthly sent",
      meta.monthly_sent != null ? `£${meta.monthly_sent.toFixed(0)}` : "—",
    ],
    [
      "Monthly limit",
      meta.monthly_limit != null ? `£${meta.monthly_limit}` : "—",
    ],
    ["XGBoost", ml.xgb_score != null ? `${ml.xgb_score}/100` : "—"],
    ["IsoForest", ml.iso_score != null ? `${ml.iso_score}/100` : "—"],
    ["Outlier", ml.is_outlier != null ? (ml.is_outlier ? "yes" : "no") : "—"],
  ];
